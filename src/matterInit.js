...
class MatterHandler {
  ...
  init() {
    ...
    // Matter 0.19 runs one fixed update per display frame. Accumulate elapsed
    // time ourselves so 60/120/144 Hz displays all simulate 60 ticks per second.
    let previous = performance.now(),
      accumulator = 0;
    const step = 1000 / 60;
    const tick = (now) => {
      accumulator += Math.min(100, Math.max(0, now - previous));
      previous = now;
      while (accumulator >= step) {
        if (this.engine.timing.timeScale > 0) Engine.update(this.engine, step);
        accumulator -= step;
      }
    ...
    if (window.hostConnection) {
      this.clockWorker = new Worker("./src/physics-clock.js");
      this.clockWorker.onmessage = () => tick(performance.now());
      window.addEventListener("pagehide", () => this.clockWorker.terminate(), {
        once: true,
      });
    } else this.runner.frameRequestId = requestAnimationFrame(tick);
    this.running = true;
  }
