...
(function(){
  ...
  function baseStage(id,n,w,h){
    return {id,name:CAMPAIGN_TEMPLATE_CONFIG[id].name,playerCount:n,width:w,height:h,map:makeBase(w,h),blocks:[],lasers:[],buttons:[],keys:[],doors:[],jumppads:[],boundaries:[],texts:[],bindPlayers:false,shieldRule:null,spawn:pos(2,h-3),description:''};
  }
  ...
  function buildVersusCampaign(teamCounts){
    ...
    const laneHeight=Math.max(...s1.map(s=>s.height),...s2.map(s=>s.height))+4,team2Y=laneHeight,totalH=laneHeight*2-4,totalW=cursor-5,map=blank(totalW,totalH),all={blocks:[],lasers:[],buttons:[],keys:[],doors:[],jumppads:[],boundaries:[],texts:[]},spawns={team1:{},team2:{}},stageMeta={team1:{},team2:{}};
    function merge(team,stages,yOffset){stages.forEach((stage,index)=>{const sn=index+1,ox=xOffsets[index];for(let y=0;y<stage.height;y++)for(let x=0;x<stage.width;x++)if(stage.map[y][x])map[y+yOffset][x+ox]=stage.map[y][x];const moved=offsetBlueprint(stage,ox,yOffset,team,sn);['blocks','lasers','buttons','keys','doors','jumppads','boundaries','texts'].forEach(k=>all[k].push(...moved[k]));spawns[team][sn]=moved.spawn;stageMeta[team][sn]={bindPlayers:stage.bindPlayers,shieldRule:stage.shieldRule,shields:stage.shields||[],name:stage.name,description:stage.description,bounds:{x:ox,y:yOffset,w:stage.width,h:stage.height}};});}
